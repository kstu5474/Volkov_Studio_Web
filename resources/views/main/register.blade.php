...
<script>
    ...
    var emailValid = 0;
    var passwordValid = 0;
    var passwordRwValid = 0;
    var birthValid = 0;
    var agree_rule = document.getElementById('agree_rule').value;
    var agree_user = document.getElementById('agree_user').value;

    console.log(agree_rule);
    ...
    function sendit()
    {
        if(idValid === 1 && emailValid === 1 && passwordValid === 1 && passwordRwValid === 1 && birthValid === 1 agree_rule)
        {
            alert("회원가입이 완료 되었습니다.");
        }
        else
        {
            alert("화면에 표시된 사항을 확인 후 다시 진행 바랍니다.");
        }
        return false;
    }
    ...
    function existId() {
        ...
        //데이터베이스 ajax왔다갔다 해서 결과 가져오기
        var result = 0;
        $.ajax({
            type: 'POST',
            url: "{{ url('/register/existid') }}",
            data: {
                userid: userid,
                _token: '{{ csrf_token() }}'
            },
            ...
            success: function(response){
                ...
                var returnResult = ajaxId(existcode, count);

                console.log("아이디 형식 : " + existcode + ", 중복확인 : " + count + ", 최종 : " + returnResult);

                if(returnResult == 1)
                {
                    alert(userid + "는 사용 가능한 아이디 입니다.");
                    idValid = 1;
                }
                else
                {
                    alert(userid + "는 이미 사용중이거나, 사용이 불가능 한 아이디 입니다.");
                    idValid = 0;
                }
            },
            error: function(error){
                alert('Ajax통신 실패 : '+ error);
                result = 0;
                return result;
            }
        });
    ...
    //아이디 중복비교 ajax 통신 후처리
    function ajaxId(existcode, count)
    {
    ...
    function existEmail() {
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                var returnResult = ajaxEmail(existcode, count);

                console.log("아이디 형식 : " + existcode + ", 중복확인 : " + count + ", 최종 : " + returnResult);

                if(returnResult == 1)
                {
                    alert(useremail + "는 사용 가능한 이메일 입니다.");
                    emailValid = 1;
                }
                else
                {
                    alert(useremail + "는 이미 사용중이거나, 사용이 불가능 한 이메일 입니다.");
                    emailValid = 0;
                }
            },
            error: function(error){
    ...
    //아이디 중복비교 ajax 통신 후처리
    function ajaxEmail(existcode, count)
    {
        if(existcode == 0 && count == 0)
        {
            return 1;
    ...
    //비밀번호 유효성 검사
    function passwordVality() {
        var password = document.getElementById('userpassword').value;
        var resultVality = document.getElementById('existspassword');

        // 비밀번호 유효성 체크 로직 (예: 정규식 사용)
        var isValid = /^(?=.*[a-zA-Z])(?=.*[\d!@#$^&*])[a-zA-Z\d!@#$^&*]{4,15}$/.test(password);
        
        if(isValid)
        {
            resultVality.textContent = "비밀번호가 사용 가능합니다.";
            resultVality.style.color = "green";
            passwordValid = 1;
        }
        else
        {
            resultVality.textContent = "비밀번호가 사용이 불가능 합니다.(4~15글자, 영어대/소문자, 특수문자(!@#$^&*)만 사용 가능";
            resultVality.style.color = "red";
            passwordValid = 0;
        }
    }

    function samePasswordVality(){
        var password = document.getElementById('userpassword').value;
        var passwordrw = document.getElementById('rewritepassword').value;
        var resultVality = document.getElementById('samepassword');

        if(password === passwordrw)
        {
            resultVality.textContent = "비밀번호가 동일합니다.";
            resultVality.style.color = "green";
            passwordRwValid = 1;
        }
        else
        {
            resultVality.textContent = "비밀번호가 동일하지 않습니다.";
            resultVality.style.color = "red";
            passwordRwValid = 0;
        }
    }

    function existsBirth(){
        var birth = document.getElementById('userbirth').value;
        var resultVality = document.getElementById('birthvalid');

        var datePattern = /^\d{4}-\d{2}-\d{2}$/;

        if(datePattern.test(birth))
        {
            resultVality.textContent = "";
            resultVality.style.color = "green";
            birthValid = 1;
        }
        else
        {
            resultVality.textContent = "생년월일을 적어주시기 바랍니다..";
            resultVality.style.color = "red";
            birthValid = 0;
        }
    }
...
<body>
    <table>
        <tr>
            <td>
                <textarea name="rule_index" id="rule_index" cols="55" rows="10">약관에 들어가는 내용1.</textarea>
                <br>
                약관에 동의합니다. &nbsp; <input type="checkbox" name="agree_rule" id="agree_rule">
                <br><br><br>
                <textarea name="rule_user" id="rule_user" cols="55" rows="10">약관에 들어가는 내용2.</textarea>
                <br>
                약관에 동의합니다. &nbsp; <input type="checkbox" name="agree_user" id="agree_user">
            </td>
            <td>
                <header align="center">
    ...
    <fieldset style="width: 40%">
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    <button onclick="existId()">중복체크</button>
                </td>
            </tr>
            <tr>
                <td>
                ...
                <td>
                    <button onclick="existEmail()">중복체크</button>
                </td>
            </tr>
            <tr>
                <td>
                ...
                <td>
                    <input type="password" name="userpassword" id="userpassword" oninput="passwordVality()">
                </td>
                <td>
                    <span name="existspassword" id="existspassword"></span>
                </td>
            </tr>
            <tr>
                <td>
                ...
                <td>
                    <input type="password" name="rewritepassword" id="rewritepassword" oninput="samePasswordVality()">
                </td>
                <td>
                    <span name="samepassword" id="samepassword"></span>
                </td>
            </tr>
            <tr>
                <td>
                ...
                <td>
                    <input type="text" name="userbirth" id="userbirth" oninput="existsBirth()">
                </td>
                <td>
                    <span name="birthvalid" id="birthvalid"></span>
                </td>
            </tr>
        </table>
        <br><br>
        <button onclick="sendit();">회원가입</button>&nbsp;&nbsp;<button onclick="gotomain()">메인화면으로</button>
    </fieldset>
</body>
</html>